...
namespace axlt::editor {
	...
	ResourceDatabase::ResourceDatabase() :
		...
		for (uint32_t i = 0; i < resourceFileSystem.files.GetSize(); i++) {
			...
			const uint32_t targetImportVersion = GetCurrentImportVersion( file.Extension() );
			if (targetImportVersion == 0xFFFFFFFF) {
				continue;
			}
		...
		for (uint32_t i = 0; i < resourceFileSystem.files.GetSize(); i++) {
			...
			if (importHashFile != nullptr) {
				...
				uint32_t importVersion = importHashJson[ "importVersion" ].GetInt();

				if (resourceHash != md5 || importHash != importMd5 || importVersion != targetImportVersion) {
	...
	void ResourceDatabase::ImportFile( File& file, const String& md5, const String& importMd5, const String& guid ) {
		...
		val.SetInt( GetCurrentImportVersion( file.Extension() ) );
		importHashesJson.AddMember( "importVersion", val, importHashesAllocator );
